...
void trade(int current,struct player **players,struct property **properties,int amntProperties,int amntPlayers)
{
	...
	if(result == 1)
	{
		...
		if(answer == 1)
		{
			...
			else
			{
				...
				,(player[(properties[prop])->owner])->id,(properties[select])->name,(properties[prop])->name);
				int answer = 0;
				char want[2] = {'y','n'};
	...
	else
	{
		...
		parseInput(1,NULL,&selPla,NULL,range,NULL,NULL);
		selPla--;
		(player[current])->money -= money;
		(player[selPla])->money =+ money;
	}			
}
